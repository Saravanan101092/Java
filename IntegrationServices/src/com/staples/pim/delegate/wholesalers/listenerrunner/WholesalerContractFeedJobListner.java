...
public class WholesalerContractFeedJobListner extends BatchJobListener {
	...
	public void afterJob(JobExecution jobExecution) {
		...
		String wholesalerContractFeedDoneDir = jobParameters.getString(RunSchedulerWholesalerContract.WHOLESALER_CONTRACT_FEED_DONE_DIR);
		String wholesalerContractFeedBadDir = jobParameters.getString(RunSchedulerWholesalerContract.WHOLESALER_CONTRACT_FEED_BAD_DIR);
		...
		String errorReportFile = jobParameters.getString(RunSchedulerWholesalerContract.WHOLESALER_CONTRACT_FEED_REPORT_FILE);
		...
		try {
			if (BatchStatus.COMPLETED.equals(jobExecution.getStatus())) {
				...
				Util.getInstance().fileSFTPToStepInboundDir(ftpConnectionBean, outputFileList, RunSchedulerWholesalerContract.PUBLISH_ID);
				moveToDestnationDir(inputFile, wholesalerContractFeedDoneDir);
				traceLogger.info(clazzname, "afterJob", "Output STEP XML moved to :" + wholesalerContractFeedDoneDir);
			}
			/*
			 ...
			 */
				 ...
				 */
				moveToDestnationDir(inputFile, wholesalerContractFeedBadDir);
				traceLogger.info(clazzname, "afterJob", "Output STEP XML moved to :" + wholesalerContractFeedBadDir);
				String msg = "Batch Status Failed - Feed file (" + inputFile + ") moved to " + wholesalerContractFeedBadDir + " directory";
				traceLogger.error(clazzname, "afterJob", msg);
				ehfLogger.error(clazzname, "afterJob", msg);
				RunSchedulerWholesalerContract.itemTraceLogger.error(msg);
				IntgSrvUtils.alertByEmail(new Exception(msg), clazzname, RunSchedulerWholesalerContract.PUBLISH_ID);
			}

			Util.getInstance().sendSummaryEmailReport("WholesalerContract", inputFileNameAndRecordCount, outputFileList, errorReportFile,
	...
	private List<String> getOutputFiles(JobExecution jobExecution, String outputFile) {
		...
		for (Entry<String, Object> entry : parameterMap) {
			if (entry.getKey().contains(RunSchedulerWholesalerContract.FEED_FILE_BEGIN) && entry.getKey().contains(outputFile)
	...
	public void beforeJob(JobExecution jobExecution) {
		...
		traceLogger.info(clazzname, "beforeJob", "ENTER/.../EXIT: RunSchedulerWholesalerContract - initialization <- job params");
	}
	...
	private FTPConnectionBean getSFTPDestDetails() {
		...
		connectionsBean.setDestinationUrl(IntgSrvPropertiesReader
				.getProperty(IntgSrvAppConstants.WHOLESALER_CONTRACT_FEED_SFTP_TARGET_DIR_STEP));
